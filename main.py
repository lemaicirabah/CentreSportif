...
def submit_registration(first_name, last_name, username, password, email, address, tel, role, window):
    if not username or not password or not email or not role:
        messagebox.showerror("Error", "All fields are required!")
        return

    try:
        # Passer les arguments dans le bon ordre à la fonction add_user
        database.add_user(first_name, last_name, username, password, email, address, tel, role)
        window.destroy()
...
class SportCenterApp(tk.Tk):
    ...
    def register_user(self):
        ...
        ttk.Label(reg_win, style="My.TLabel", text="Last name:").grid(row=2, column=0, padx=10, pady=10)
        last_name_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        last_name_entry.grid(row=2, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Username:").grid(row=3, column=0, padx=10, pady=10)
        username_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        username_entry.grid(row=3, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Password:").grid(row=4, column=0, padx=10, pady=10)
        password_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        password_entry.grid(row=4, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Email:").grid(row=5, column=0, padx=10, pady=10)
        email_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        email_entry.grid(row=5, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Address:").grid(row=6, column=0, padx=10, pady=10)
        address_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        address_entry.grid(row=6, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Telephone:").grid(row=7, column=0, padx=10, pady=10)
        tel_entry = ttk.Entry(reg_win, font=("Arial", 14), width=20)
        tel_entry.grid(row=7, column=1, padx=10, pady=10)

        ttk.Label(reg_win, style="My.TLabel", text="Role:").grid(row=8, column=0, padx=10, pady=10)
        role_combo = ttk.Combobox(reg_win, values=["member", "instructor", "admin"], font=("arial", 14), width=19)
        role_combo.grid(row=8, column=1, padx=10, pady=10)
        role_combo.current(0)

        ttk.Label(reg_win, style="My.TLabel", text="").grid(row=9, column=0, padx=10)
        ...
        ttk.Button(reg_win, style="My.TButton", text="Register", command=lambda: submit_registration(
                    ...
                    tel_entry.get(),
                    role_combo.get(),
                    reg_win), width=10).grid(row=9, column=1, pady=10)
